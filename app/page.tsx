...
function LaughStats({post,compact=false}:{post:Post;compact?:boolean}){return <div className="laugh-stats"><strong>ㅋ {post.kCount.toLocaleString('ko-KR')}</strong><span title={`수집한 댓글 ${post.commentCount}개 기준 · 통계적인 신뢰 확률은 아닙니다`}>댓글당 ㅋ {(post.commentCount? (post.kCount/post.commentCount).toFixed(2):'—')}</span>{!compact&&<small>댓글 {post.commentCount.toLocaleString('ko-KR')}개{post.commentsPartial?' · 일부':''}</small>}</div>}
...
export default function Home(){
 ...
 return <main>
  ...
  {feed?.collectionMessage&&feed.collectionStatus!=='complete'&&<p className="collection-notice" role="status">{feed.collectionMessage}</p>}<TabsContent value={tab} key={tab}>{error&&!feed?<div className="empty" role="alert"><span className="big-k">ㅋ</span><p>불러오지 못했습니다</p><button onClick={()=>setRetry(v=>v+1)}>다시 시도</button></div>:posts.length===0?<div className="empty" role="status"><span className="big-k" aria-hidden="true">ㅋ</span><p>{feed?.collectionMessage?'수집된 글이 없습니다':tab==='humor'?'ㅋ수집중':'아직 수집한 글이 없어요'}</p></div>:<ol className="posts">{posts.map((p,i)=><li key={p.id} style={{'--source-color':sourceColors[p.source]||'#596661'} as CSSProperties} className={read.includes(p.id)?'seen':''}><Dialog open={opened===p.id} onOpenChange={open=>{if(open)openPost(p.id);else setOpened(current=>current===p.id?null:current)}}><article><div className="post-top"><span className="rank">{String(i+1).padStart(2,'0')}</span><div className="post-main">{(p.source!=='애객'||read.includes(p.id))&&<span className="source">{p.source==='애객'?'읽음':p.source+(read.includes(p.id)?' · 읽음':'')}</span>}<h2><PostTitle post={p} expanded={opened===p.id}/></h2><LaughStats post={p}/></div></div></article><DialogContent className="reader-dialog" showCloseButton={false} aria-describedby={undefined}><DialogClose className="reader-close" aria-label="닫기">×</DialogClose><nav className="reader-nav reader-top" aria-label="게시글 이동"><button onClick={()=>nextPost(-1)} disabled={activeOrder.indexOf(p.id)<=0} aria-label="이전 글">← 이전 글</button><div className="reader-heading"><DialogTitle className="reader-title">{p.title}</DialogTitle><span className="reader-counter">{activeOrder.indexOf(p.id)+1} / {activeOrder.length}</span></div><button onClick={()=>nextPost(1)} disabled={activeOrder.indexOf(p.id)>=activeOrder.length-1} aria-label="다음 글">다음 글 →</button></nav><LaughStats post={p} compact/>{opened===p.id&&<div className="reader-body" data-reader-active="true"><Body post={p}/><Comments post={p}/>{(p.matchedPosts?.length?p.matchedPosts:[{url:p.url,source:p.source}]).map(m=><a key={m.url} className="original" href={m.url} target="_blank" rel="noopener noreferrer">{m.source==='애객'?'':m.source+' '}원문 보기 ↗</a>)}</div>}</DialogContent></Dialog></li>)}</ol>}</TabsContent></Tabs>
 </main>
}
